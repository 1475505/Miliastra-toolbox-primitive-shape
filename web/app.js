  ...
  const defaultExportBaseName = (taskImageName.replace(/\.[^.]+$/, "").trim()) || "shaper_result";
  ...
  const exportNameInput = $("exportFileName");
  ...
  function currentExportBaseName() {
    const rawName = exportNameInput ? exportNameInput.value : "";
  ...
  function exportFileName(ext) {
    return `${currentExportBaseName()}.${ext}`;
  }
  ...
  function cssSolidColor(color) {
    ...
    if (typeof color === "string") {
      return color.trim() || "#ffcc00";
    }
    return "#ffcc00";
  }

  function svgFillColor(color) {
    if (Array.isArray(color)) {
      return `rgb(${color[0]},${color[1]},${color[2]})`;
    }
    if (typeof color === "string") {
  ...
  function buildJsonExportText() {
    return JSON.stringify(buildJsonExportPayload(), null, 2);
  }

  function buildCssExportText() {
    const elements = renderOrderedElements();
    const lines = [];
    const exportBaseName = currentExportBaseName();
    ...
    lines.push(`  background: ${outputHasTransparency ? "transparent" : "#ffffff"};`);
    ...
    elements.forEach((element, index) => {
      const center = imagePointToElementCenter(element);
      const type = normalizeType(element.type);
      const angle = element.rotation ? -Number(element.rotation.z || 0) : 0;
      const rawAlpha = Number(element.alpha);
      ...
      if (type === "ellipse") {
        width = (element.size.rx || (element.size.width || 0) / 2) * pixelPerUnit * 2;
        height = (element.size.ry || (element.size.height || 0) / 2) * pixelPerUnit * 2;
        extraStyle = "  border-radius: 50%;";
      } else if (type === "triangle") {
        width = (element.size.width || 0) * pixelPerUnit;
        height = (element.size.height || 0) * pixelPerUnit || (width * Math.sqrt(3) / 2);
        extraStyle = "  clip-path: polygon(50% 0%, 0% 100%, 100% 100%);";
        transformOrigin = "50% 66.6667%";
      } else {
        width = (element.size.width || 0) * pixelPerUnit;
        height = (element.size.height || 0) * pixelPerUnit;
      }
  ...
  async function init() {
    if (exportNameInput) exportNameInput.value = defaultExportBaseName;
  ...
  if ($("btnExportSVG")) {
    $("btnExportSVG").addEventListener("click", async () => {
      ...
      elements.forEach((element) => {
        ...
        const rawAlpha = Number(element.alpha);
        const alpha = Number.isFinite(rawAlpha) ? Math.max(0, rawAlpha) : 0.5;
        const color = svgFillColor(element.color);
        ...
        if (type === "triangle") {
          const width = (element.size.width || 0) * pixelPerUnit;
          const height = (element.size.height || 0) * pixelPerUnit || (width * Math.sqrt(3) / 2);
          const x1 = center.x;
          const y1 = center.y - (2 * height / 3);
          const x2 = center.x - (width / 2);
          const y2 = center.y + (height / 3);
          const x3 = center.x + (width / 2);
          const y3 = y2;
  ...
  if ($("btnExportJSON")) {
    $("btnExportJSON").addEventListener("click", async () => {
      try {
        await saveBlob(
          new Blob([buildJsonExportText()], { type: "application/json" }),
          exportFileName("json"),
        );
      } catch (error) {
        alert(`导出失败: ${error && error.message ? error.message : error}`);
  ...
  if ($("btnExportCSS")) {
    $("btnExportCSS").addEventListener("click", async () => {
      try {
        await saveBlob(
          new Blob([buildCssExportText()], { type: "text/css" }),
          exportFileName("css"),
        );
      } catch (error) {
        alert(`导出失败: ${error && error.message ? error.message : error}`);
  ...
  if ($("btnCopyJSON")) {
    $("btnCopyJSON").addEventListener("click", async () => {
      try {
        ...
        setTemporaryButtonState($("btnCopyJSON"), "已复制", "is-success");
      } catch (error) {
  ...
  if ($("btnCopyCSS")) {
    $("btnCopyCSS").addEventListener("click", async () => {
      try {
        ...
        setTemporaryButtonState($("btnCopyCSS"), "已复制", "is-success");
      } catch (error) {
        alert(`复制失败: ${error && error.message ? error.message : error}`);
      }
    });
