...
| 特性 | 轮廓模式 (Outline) | 填充模式 (Fill) |
|------|-------------------|----------------|
| 引擎 | `final_shaper.py` | `fill_shaper.py` |
| 拟合方式 | 沿轮廓路径行走 | 随机采样 + 爬山优化 |
| 图元放置 | 沿轮廓依次排列 | 在前景区域内随机分布 |
| 支持图元 | 椭圆、矩形 | 圆形（椭圆）、矩形、三角形 |
| 颜色 | 可选固定色 | 从图片采样颜色 |
| 透明度 | 全局 | 逐图元自适应 |
...
轮廓/装饰物模式使用 `final_shaper.py` 实现路径行走拟合，并支持**装饰物预设**：
